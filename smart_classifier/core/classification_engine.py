...
class ClassificationEngine:
    ...
    def execute_plan(
        ...
        now with full support for Pause, Resume, and Cancel.
        """
        ...
        # Step 1: Ensure all state flags are reset for a clean run.
        self.reset_state()

        logger.info(f"Executing plan for {total_files} files with strategy: {duplicate_strategy.name}")

        # Prepare the transaction log for a potential undo operation.
        UndoManager.clear_log()

        max_workers = get_optimal_thread_count()
        files_processed = 0

        # Use the ThreadPoolExecutor to manage our worker threads.
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            # Submit all file-moving tasks to the executor's queue.
            future_to_source = {
                executor.submit(safe_move, source_path, dest_dir, duplicate_strategy): source_path
                for source_path, dest_dir in plan
            }

            # Process futures as they complete for real-time progress.
            for future in as_completed(future_to_source):
                # --- CORE PAUSE/RESUME/CANCEL LOGIC ---
                # This is the "checkpoint" the worker thread hits before processing each file.
                # If self._pause_event is cleared (paused), this line will block indefinitely
                # with zero CPU usage until self.resume() is called.
                self._pause_event.wait()

                # If self.cancel() was called, the flag will be True.
                if self._is_cancelled:
                    logger.warning("Operation cancelled by user. Halting processing.")
                    # A robust extra step: try to cancel any futures that haven't started.
                ...
                try:
                    # We get the result of the file-moving operation.
                    # A small timeout makes the loop more responsive to cancellation.
                    status, final_dest_path = future.result(timeout=0.1)

                    # If the move was successful, log it for the Undo feature.
                    if status == "MOVED":
